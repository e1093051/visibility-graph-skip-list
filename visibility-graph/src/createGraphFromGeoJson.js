...
export function processPoint (p, pointsLen, scan, visibilityGraph) {
  ...
  for (let ii = 0; ii < pointsLen; ii++) {
    // if (p.nodeId === 1206) console.log(ii)
    ...
    if (prev === null || ccw(p, prev, p2) !== 0 || !onSegment(p, prev, p2)) {
      if (openEdges.keys.length === 0) {
        isVisible = true
      } else if (!edgeIntersect(p, p2, openEdges.keys.minKey().edge)) {
        isVisible = true
      }
    } else if (!prevVisible) {
      isVisible = false
      ...
      isVisible = true
      for (const [e] of openEdges.keys.findEntriesFromMin()) {
